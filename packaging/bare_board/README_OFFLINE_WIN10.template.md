...
3. Remove the incompatible canonical J-Link 9.56 installation left by package r12, if present.
4. Install Nordic Command Line Tools (`nrfjprog`) with its bundled, validated SEGGER J-Link 7.94e.
5. Validate J-Link 7.94e with `nrfjprog --version --jdll <path>` and save that DLL path in workstation `config.json`.
6. Run its `USBDriver\InstDrivers.exe /silent`; if it exposes loose `JLink*.inf` files, verify/register them with `pnputil`.
7. Launch the workstation setup installer.
